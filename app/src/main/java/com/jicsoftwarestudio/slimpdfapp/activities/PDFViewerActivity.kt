...
import com.jicsoftwarestudio.slimpdfapp.helper.ScrollHandle
...
class PDFViewerActivity : AppCompatActivity() {
    ...
    public override fun onCreate(bundle: Bundle?) {
        ...
        val intent = intent
        pdfFileLocation =
            intent.getStringExtra(com.jicsoftwarestudio.slimpdfapp.data.Constants.PDF_LOCATION)
        showRemoveAds = intent.getBooleanExtra(
            com.jicsoftwarestudio.slimpdfapp.data.Constants.SHOW_REMOVE_ADS,
            false
        )
        uri = intent.data
    ...
    public override fun onActivityResult(i: Int, i2: Int, intent: Intent?) {
        ...
        if (i == 7 && i2 == -1) {
            pdfView!!.jumpTo(intent!!.getIntExtra(com.jicsoftwarestudio.slimpdfapp.data.Constants.PAGE_NUMBER, pdfView!!.currentPage) - 1, true)
        }
    }
    ...
    private fun toggle() {
        ...
        show()
        if (AUTO_HIDE) {
            delayedHide(com.jicsoftwarestudio.slimpdfapp.data.Constants.AUTO_HIDE_DELAY_MILLIS)
        }
    }
